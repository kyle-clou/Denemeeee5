  ...
  Database.findOne({ SunucuID: msg.guild.id, userID: kullanici.id}, async (err, res) => {
    ...
    res.ToplamMesaj.forEach((val, key) => mesajSayisi.push({ kanal: key, sayi: val }));
      Array.from(res.ToplamSes).forEach(([key, value]) => {
       ...
       // if (msg.guild.channels.cache.get(key).parentID === "") return weeklyAlone += value;
...
const settings = require('../../staff')
    ...
    <@${kullanici.user.id}> üyesinin bilgileri:

    **En yüksek rolü:**
    ${kullanici.roles.highest} Rolü.    

    **❯ Kullanıcının Genel Bilgileri:**
    \`•\` Kullanıcı: <@${kullanici.user.id}> (${kullanici.user.id})
    \`•\` Geçirdiği toplam süre: \`${totalVoice}\`
    \`•\` Geçirdiği haftalık süre: \`${parseTime(weeklyVoice)}\`

    \`•\` Haftalık vakit geçirdiği kanallar: \`${allChannels.length} kanal\`
    \`•\` Haftalık mesaj sayısı: \`${weeklyMessage}\`
    \`•\` Toplam mesaj sayısı: \`${totalMessage} mesaj atmış.\`
    
    **❯ Ses Kanalları Sıralaması:**
    \`(${mostActiveChannels.length} Kanalda Bulunmuş)\`
    ${mostActiveChannels.join("\n") || "Bulunamadı."}
    
    **❯ Genel Sesli Kanal Bilgileri:**
    ...
    **❯ Mesaj Kanalları Sıralaması:**
    ${ilk5.join("\n") || 'Bulunamadı.'}

    **❯ En Çok Vakit Geçirdiği Kategoriler:**
    ${pubxparent.join("\n") || "Bulunamadı."} 
...
module.exports.help = {
  name: "stats",
  alias: ["me","Stat","STATS","Stats","Me","ME","STAT","stats", "ses"]
};
